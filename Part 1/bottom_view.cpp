...
vint bottom_view(node* root) {
    ...
    while (!q.empty()) {
        ...
        q.pop();
        node* front_end = temp.first;
        int hd = temp.second;
        // if (top_nodes.count(hd) == 0) {
        //     top_nodes[hd] = front_end->data;
        // }
        top_nodes[hd] = front_end->data;
        if (front_end->left) {
            q.push(make_pair(front_end->left, hd - 1));
        }
        if (front_end->right) {
            q.push(make_pair(front_end->right, hd + 1));
        }
    }
    for (auto it : top_nodes) {
